...
def get_ltor_masks_and_position_ids(
    ...
    if len(loss_mask_alternating_individual) > 0:
        ...
        # first, lets seperate the packed examples (identify the boundaries)
        split_id = eod_token
        sep = (data == split_id)  # [B, S_full] bool
        
        for id in loss_mask_alternating_individual:
            eq = data == id
            
            # now calculate occurances based on the seperator
            glob = eq.long().cumsum(dim=1)
            last_sep_glob = torch.cummax(glob * sep.long(), dim=1).values # maximum for each segment
            cs = glob - last_sep_glob
            
            # misc
            B, S = cs.shape
            device = cs.device
            
            end_of_seg = torch.zeros((B, S), dtype=torch.bool, device=device)
            end_of_seg[:, :-1] = sep[:, 1:] # true if next token is a seperator
            end_of_seg[:, -1] = True # segment always ends at the end of the row
            
            idx = torch.arange(S, device=device).unsqueeze(0).expand(B, S)
            
            BIG = 2 * S # sentinel value
            
            end_idx = torch.where(end_of_seg, idx, torch.full_like(idx, BIG)) # use sentinel so that cummin works
            
            # for each position find where the next segment ends
            next_end_idx = torch.flip(
                torch.cummin(torch.flip(end_idx, dims=[1]), dim=1).values,
                dims=[1]
            )
            
            seg_total = cs.gather(1, next_end_idx)
            
            seg_exact2   = (seg_total == 2)
            seg_even_gt2 = (seg_total > 2) & ((seg_total % 2) == 0)
            seg_odd      = (seg_total % 2) == 1
            
            # Original alternating-individual (within each segment): odd occurrences only.
            odd_pos = eq & ((cs % 2) == 1)

            # Symmetric "pair masking" for same-token pairs within each segment:
            # inside_all: True for positions when a pair is "open" (including opener position),
            # based on parity of cs (segment-local).
            inside_all = (cs % 2) == 1

            # Include closer token by shifting from previous position, but DO NOT leak across separators.
            # IMPORTANT: gate on previous token NOT being a separator.
            pair_all = inside_all.clone()
            pair_all[:, 1:] |= (inside_all[:, :-1] & ~sep[:, :-1])

            # Identify the last open interval (the last pair) within each segment: opener has cs == seg_total - 1.
            inside_last = inside_all & (cs == (seg_total - 1))
            pair_last = inside_last.clone()
            pair_last[:, 1:] |= (inside_last[:, :-1] & ~sep[:, :-1])

            # For even_gt2 segments, apply pair masking for all pairs except the last.
            pair_except_last = seg_even_gt2 & (pair_all & ~pair_last)

            # For the last pair in even_gt2 segments, apply "original" masking only to the last opener token.
            last_opener = eq & (cs == (seg_total - 1))

            # Combine:
            final_mask = torch.zeros_like(eq, dtype=torch.bool)
            final_mask |= (seg_exact2 | seg_odd) & odd_pos
            final_mask |= seg_even_gt2 & last_opener
            final_mask |= pair_except_last

            # Do not mask the separator token itself as part of this rule.
            final_mask &= ~sep

            # Apply (align with out_seq)
            block_loss |= final_mask[:, 1:]

            # Warn on any segment with odd number of occurrences (unclosed).
            if (seg_total[end_of_seg] % 2 == 1).any():
                print_rank_0(
                    f"WARNING: Found unclosed alternating individual token {id} in one or more samples "
                    f"(within a {split_id}-split segment)."
                )
                unclosed_pair = True
